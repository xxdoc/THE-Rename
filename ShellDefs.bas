...
Public Function GetPIDLFromPath(hwndOwner As Long, sPath As String) As Long
  Dim isfDesktop As IShellFolder
  Dim pchEaten As Long
  Dim pidl As Long

  If SHGetDesktopFolder(isfDesktop) = NOERROR Then
    If isfDesktop.ParseDisplayName(hwndOwner, 0, StrConv(sPath, vbUnicode), pchEaten, pidl, 0) = NOERROR Then
